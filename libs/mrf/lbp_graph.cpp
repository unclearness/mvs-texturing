...
ENERGY_TYPE LBPGraph::compute_energy() {
    ...
    for (std::int64_t vertex_idx = 0; vertex_idx < static_cast<std::int64_t>(vertices.size()); ++vertex_idx) {
        Vertex const & vertex = vertices[vertex_idx];
        energy += vertex.data_cost;
    }

    #pragma omp parallel for reduction(+:energy)
    for (std::int64_t edge_idx = 0; edge_idx < static_cast<std::int64_t>(edges.size()); ++edge_idx) {
